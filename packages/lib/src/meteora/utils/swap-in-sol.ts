...
export const initialize_swap_in_sol = async (
  ...
  const swapForY = mint.equals(nativeToMintPool.tokenX.mint.address);
  const amount = to_bn(parse_amount(args.amount, nativeMint.decimals));
  const binArrays = await nativeToMintPool.getBinArrayForSwap(swapForY);
  ...
  return { swapQuote, transaction, swapForY };
};
